...
(function () {
  ...
  function paint(){
    ...
    if (cap) cap.innerHTML = active
      ? '<span class="nm" style="color:' + C[active] + '">' + R[active].name + '</span><span class="sub">' + R[active].n + ' — ' + (active === 'douro' ? '21 concelhos' : active === 'dao' ? '17 concelhos' : 'Monção e Melgaço') + '</span>'
      : '<span class="nm">Portugal</span><span class="sub">Três regiões</span>';
  ...
  function intro(){
    return `<h1 class="serif h2">Xisto e granito<i>Três regiões · oito hectares</i></h1>
      <p>As vinhas não foram escolhidas por região, mas por solo. Xisto micáceo no Douro, granito no Dão e no vale do Minho — as mesmas castas dariam vinhos diferentes em cada um deles.</p>
      <p>Carregue numa região do mapa. As manchas mais fortes são os concelhos onde estão as vinhas.</p>`;
  }
  function body(reg){
  ...
  function render(reg){
    ...
    p.innerHTML = `<div class="fade">${reg ? body(reg) : intro()}
      <div class="switch">
        ...
        <button type="button" data-go="all">Ver tudo</button>
      </div></div>`;
